...
| R-08 | Ledger-validated transitions only | XRPL JSON-RPC client plus escrow validation and confirmation routes | XRPL client/service unit tests and confirmation integration suites | `docs/state-machines.md`, `docs/api-spec.md` | done |
...
| R-10 | Backend enforces invariants (frontend untrusted) | role gates, server-fetched XRPL evidence, fighter-only signup, admin provisioning, and typed two-field confirmation clients | security/integration suites, frontend client tests, browser E2E, CI gitleaks job | `docs/api-spec.md`, `docs/ci-cd.md`, `frontend/README.md` | done |
...
Current deterministic evidence is recorded in `docs/testnet-release-readiness.md`. Release hardening is complete; live Xaman/XRPL smoke validation remains operator-run because it requires environment-managed credentials and funded Testnet accounts.
